...
export const ManageEducation: React.FC = () => {
  ...
  const handleEdit = (item: any) => {
    ...
    setIcon(item.icon || 'GraduationCap');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const payload = { school, degree, timeline, description, icon, order };
      if (editingId) {
  ...
  const handleReorder = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      await api.put('/education/reorder', { 
        items: [
    ...
    <div className="max-w-4xl">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-text-muted text-sm mt-1">Add, update, or remove education history.</p>
        </div>
        <button onClick={() => { resetForm(); setIsAdding(!isAdding); }} className="px-4 py-2 bg-primary text-white rounded-xl hover:bg-primary-dark transition-all flex items-center gap-2 text-sm font-medium">
          {isAdding ? 'Cancel' : <><Plus size={18} /> Add Education</>}
        </button>
      </div>

      {isAdding && (
        <form onSubmit={handleSubmit} className="glass-panel p-6 sm:p-8 rounded-2xl border border-white/10 mb-8 space-y-6">
          <h2 className="text-lg font-semibold">{editingId ? 'Edit Education' : 'Add New Education'}</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-muted">School / University</label>
              <input value={school} onChange={(e) => setSchool(e.target.value)} required className="w-full px-4 py-3 rounded-xl bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 focus:outline-none focus:ring-2 focus:ring-primary/50 text-text" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-muted">Degree / Field</label>
              <input value={degree} onChange={(e) => setDegree(e.target.value)} required className="w-full px-4 py-3 rounded-xl bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 focus:outline-none focus:ring-2 focus:ring-primary/50 text-text" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-muted">Timeline (e.g. 2018 - 2022)</label>
          ...
          <div className="flex justify-end pt-4">
            <button type="submit" disabled={isSaving} className="px-6 py-3 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition-all flex items-center gap-2">
              {isSaving ? <Loader2 className="animate-spin" size={18} /> : 'Save Education'}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="space-y-4">
        {items.map((item, index) => (
          <div key={item._id} className="glass-panel p-6 rounded-2xl border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-bold">{item.degree} <span className="text-text-muted font-normal text-sm ml-2">{item.school}</span></h3>
        ...
        {items.length === 0 && !isAdding && (
          <div className="text-center py-12 text-text-muted glass-panel rounded-2xl border border-white/10">No education added yet.</div>
        )}
      </div>
    </div>
  );
