...
export const AdminSettings: React.FC = () => {
  ...
  return (
    <div className="py-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-3xl border border-border dark:border-border-dark p-8 max-w-3xl">
        <div className="mb-6 flex items-center gap-3 text-text">
          <div className="p-3 rounded-2xl bg-violet-500/10 text-violet-500"><GithubIcon size={24} /></div>
          <div>
        ...
        <form className="space-y-6" onSubmit={handleSave}>
          <div className="space-y-2">
            ...
            <input id="github-username" type="text" value={githubUsername} onChange={(e) => setGithubUsername(e.target.value)} placeholder="github.com/username" className="w-full rounded-2xl border border-border bg-surface px-4 py-3 text-text focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" disabled={isLoading} />
          </div>

          <div className="space-y-2">
            <label htmlFor="github-token" className="text-sm font-medium text-text">GitHub Token</label>
            <input id="github-token" type="password" value={githubToken} onChange={(e) => setGithubToken(e.target.value)} placeholder="Personal access token (optional)" className="w-full rounded-2xl border border-border bg-surface px-4 py-3 text-text focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" disabled={isLoading} />
          ...
          <button type="submit" disabled={isLoading || isSaving} className="inline-flex items-center gap-2 rounded-2xl bg-primary px-6 py-3 text-white transition hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-60">
            <Save size={16} /> {isSaving ? 'Saving...' : 'Save GitHub Settings'}
          </button>
        </form>
      </motion.div>
    </div>
