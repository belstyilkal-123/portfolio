          ...
          {messages?.map((msg: any) => (
            <div 
              ...
              className={`p-6 rounded-2xl border transition-all ${
                msg.isRead 
                  ? 'bg-surface dark:bg-surface-dark border-border dark:border-border-dark opacity-75' 
                  : 'bg-white dark:bg-zinc-800 border-primary/30 shadow-md'
              }`}
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-sm text-text-muted mt-1">
                    From: <span className="font-medium text-text">{msg.name}</span> ({msg.email})
                  </p>
                </div>
              ...
              <div className="p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl text-text-muted mb-4">
                {msg.message}
              </div>
              ...
              {!msg.isRead && (
                <div className="flex justify-end">
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600 rounded-lg text-sm font-medium transition-colors"
                  >
                    Mark as Read
                  </button>
                </div>
