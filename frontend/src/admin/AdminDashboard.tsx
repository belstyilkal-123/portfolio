  ...
  const stats = [
    {
      ...
      value: statsLoading ? 'Loading...' : `${adminStats?.totalProjects ?? 0}`,
      icon: Briefcase,
      color: 'text-blue-500',
      bg: 'bg-blue-500/10',
    },
    {
      label: 'Live Projects',
      value: statsLoading ? 'Loading...' : `${adminStats?.liveProjects ?? 0}`,
      icon: ExternalLink,
      color: 'text-cyan-500',
      bg: 'bg-cyan-500/10',
    },
    {
      label: 'GitHub Repos',
      value: statsLoading ? 'Loading...' : `${adminStats?.totalGitHubRepos ?? 0}`,
      icon: GitBranch,
    ...
    {
      label: 'GitHub Stars',
      value: statsLoading ? 'Loading...' : `${adminStats?.totalGitHubStars ?? 0}`,
      icon: Star,
    ...
    },
    {
      ...
      value: statsLoading ? 'Loading...' : `${adminStats?.unreadMessages ?? 0}`,
      icon: MessageSquare,
    ...
    {
      label: 'Admin Users',
      value: statsLoading ? 'Loading...' : `${adminStats?.adminUsers ?? 0}`,
      icon: Users,
    ...
    <div>
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-text-muted">Here is what's happening with your portfolio today.</p>
        </div>
      </div>
      ...
      <div className="glass-panel p-8 rounded-3xl border border-white/10">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-text-muted">Quick access to the latest messages in your inbox.</p>
          </div>
          <Link
            to="/admin/messages"
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-full text-sm font-semibold hover:bg-primary-dark transition-colors"
          >
